...
global torus: true {
	...
	file mapGrid<- csv_file("../includes/BorderFar.csv",","); //landcover type grid spreadsheet
	bool randomMap<- true;
	...
	action save_map{
		ask Landscape{
			save species_of(self) to:"RndSim"+string(rep)+".shp" type:shp attributes:["name"::name,'cellCover'::cellCover,'doveSeedsDeg'::doveSeedsDeg,'doveSeedsNat'::doveSeedsNat,'starSeedsDeg'::starlingSeedsDeg,'starSeedsNat'::starlingSeedsNat,'SeedsDeg'::SeedsDeg,'SeedsNat'::SeedsNat,'totalSeeds'::totalSeeds];
		}
	}
...
experiment Batch type:batch repeat: 1 keep_seed:true until:(year=1 and day=1){
