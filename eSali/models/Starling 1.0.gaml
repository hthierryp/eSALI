...
global torus: false {
	...
	file mapGrid<- csv_file("../includes/BorderMap.csv",","); //landcover type grid spreadsheet
	bool randomMap<- true;
	...
	action create_landcovers{
		...
		loop i from: 0 to:landcoverMatrix.rows-1{
			create Landcover{
				...
				priorityStar<-int(landcoverMatrix[4,i]);
				priorityDove<-int(landcoverMatrix[5,i]);				
	...
	action save_seeds{
		ask Landscape{
			save [name,cellCover,doveSeedsDeg,doveSeedsNat,starlingSeedsDeg,starlingSeedsNat,SeedsDeg,SeedsNat,totalSeeds] to:"../models/RndSim"+string(rep)+".csv" type:csv rewrite:false;
		}
	}
	action save_map{
		ask Landscape{
			save species_of(self) to:"RndSim"+string(rep)+".shp" type:shp with:[name::"name",cellCover::"cellCover",doveSeedsDeg::"doveSeedsDeg",doveSeedsNat::"doveSeedsNat",starlingSeedsDeg::"starSeedsDeg",starlingSeedsNat::"starSeedsNat",SeedsDeg::"SeedsDeg",SeedsNat::"SeedsNat",totalSeeds::"totalSeeds"];
		}
	}
...
species Landcover{
	...
	int priorityStar; //habitat priority of landcover for starlings
	int priorityDove; //habitat priority of landcover for fruit doves
...
species Bird skills:[moving]{
	Landscape myCell;
	matrix stomachSeeds<-0 as_matrix({4,2});
	init {
		myCell<- any(Landscape overlapping self);
	}
...
species Dove parent:Bird{
	...
	action dove_moving {
		if ((hour>=6) and (hour<20)){	//between 6am and 9 pm-Day movement
			...
			Landscape targetCell <- any(Landscape at_distance distanceTarget);
			bool going;
			if targetCell.cellCover.priorityDove >= myCell.cellCover.priorityDove{
				do goto target: any_location_in(targetCell) speed:distanceTarget#m;
			}
			if targetCell.cellCover.priorityDove >0 and (targetCell.cellCover.priorityDove < myCell.cellCover.priorityDove){
				int scoreDiff <- myCell.cellCover.priorityDove-targetCell.cellCover.priorityDove;
				going<-flip(1-((1/10)*scoreDiff));
				if going=true{
					do goto target: any_location_in(targetCell) speed:distanceTarget#m;
				}
				if going=false{
					path route <- self path_to targetCell;
					list<Landscape> cellsOnPath <- (Landscape overlapping route.shape);
					list<Landscape> potentialTargets <- cellsOnPath where (each.cellCover=myCell.cellCover);
					if potentialTargets!=nil{
						Landscape newTarget<- potentialTargets closest_to targetCell;
						do goto target: any_location_in(newTarget) speed:distanceTarget#m;
					}
				}
			}
		}
	...
	action dove_pooping {
		...
		ask Landscape overlapping(self){
			doveSeedsNat<-doveSeedsNat+int(myself.stomachSeeds[3,0]);
			doveSeedsDeg<-doveSeedsDeg+int(myself.stomachSeeds[3,1]);
		}
...
species Starling parent:Bird{
	...
	action starling_moving{
		if ((hour>=6) and (hour<21)){	//between 6am and 9 pm-Day movement
			float distanceTarget <-(float(gamma_rnd(159.4168016,0.9474663)))#m;	
			Landscape targetCell <- any(Landscape at_distance distanceTarget);
			bool going;
			if targetCell.cellCover.priorityStar >= myCell.cellCover.priorityStar{
				do goto target: any_location_in(targetCell) speed:distanceTarget#m;
			}
			if targetCell.cellCover.priorityStar >0 and (targetCell.cellCover.priorityStar < myCell.cellCover.priorityStar){
				int scoreDiff <- myCell.cellCover.priorityStar-targetCell.cellCover.priorityStar;
				going<-flip(1-((1/10)*scoreDiff));
				if going=true{
					do goto target: any_location_in(targetCell) speed:distanceTarget#m;
				}
