...
global torus: true {
	...
	file mapGrid<- csv_file("../includes/CorridorBig.csv",","); //landcover type grid spreadsheet
	...
	action save_seeds{
		ask Landscape{
			save [name,cellCover,doveSeedsDeg,doveSeedsNat,starlingSeedsDeg,starlingSeedsNat,SeedsDeg,SeedsNat,totalSeeds] to:"../models/CorBigSim"+string(rep)+".csv" type:csv rewrite:false;
		}
	}
	action save_map{
		ask Landscape{
			save species_of(self) to:"CorBigSim"+string(rep)+".shp" type:shp attributes:["name"::name,'cellCover'::cellCover,'doveSeedsDeg'::doveSeedsDeg,'doveSeedsNat'::doveSeedsNat,'starSeedsDeg'::starlingSeedsDeg,'starSeedsNat'::starlingSeedsNat,'SeedsDeg'::SeedsDeg,'SeedsNat'::SeedsNat,'totalSeeds'::totalSeeds];
		}
	}
